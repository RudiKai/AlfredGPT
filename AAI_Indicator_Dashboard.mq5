...
// Added for subwindow support
int g_win = 0;
const string AAI_DASH_SHORTNAME = "AlfredAI Pane";
...
void CreateLabel(string n, string t, int x, int y, color c, int fs = FONT_SIZE_NORMAL, ENUM_ANCHOR_POINT a = ANCHOR_LEFT)
{
   ...
   ObjectCreate(0, o, OBJ_LABEL, g_win, 0, 0);
...
{
   string o = PANE_PREFIX + n;
   ObjectCreate(0, o, OBJ_RECTANGLE_LABEL, g_win, 0, 0);
   ObjectSetInteger(0, o, OBJPROP_XDISTANCE, x);
   ObjectSetInteger(0, o, OBJPROP_YDISTANCE, y);
...
void CreatePanel()
{
   ...
   ObjectCreate(0, bg_name, OBJ_RECTANGLE_LABEL, g_win, 0, 0);
...
int OnInit()
{
   IndicatorSetString(INDICATOR_SHORTNAME, AAI_DASH_SHORTNAME);
   g_win = ChartWindowFind(0, AAI_DASH_SHORTNAME);
   if(g_win < 0) g_win = 1;
